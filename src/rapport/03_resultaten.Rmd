...
benthos_afdw_zeeschelde_durme %>%
  ...
  st_drop_geometry() %>%
  count(jaar, tidaal) %>%
  gt(caption = "(ref:spatial-steekproefgrootte-tidaal)")
```
...
benthos_design %>%
  ...
  summarize(
    ...
    vartype = "ci",
    level = 0.95)) %>%
  mutate(methode = "Horvitz-Thomson") -> svytotal_jaar
```
...
hg_optim_estimates <- hg_optim %>%
  ...
  mutate(overall_mean = (1 - plogis(theta_hurdle)) * (1 / theta_conditional))
...
hg_summary <- hg_samples %>%
  ...
  mutate(
    ...
    log_gamma_part.2018 = log(1/beta.2018),
    sum_logparts.2018 = log_zero_part.2018 + log_gamma_part.2018,
    log_zero_part.2019 = log(1 - plogis(betazi.2019)),
    log_gamma_part.2019 = log(1/beta.2019),
    sum_logparts.2019 = log_zero_part.2019 + log_gamma_part.2019
    ) %>%
  select(starts_with("sum")) %>%
  pivot_longer(cols = everything()) %>%
  separate(name, c("name", "jaar"), sep = "\\.") %>%
  group_by(jaar) %>%
  summarize(mzb_tonnage = exp(median(value)),
            mzb_tonnage_low = exp(quantile(value, 0.025)),
            mzb_tonnage_upp = exp(quantile(value, 0.975))
            )
# convert to tonnes
...
hg_salzone_summary <- hg_salzone_samples %>%
  ...
  mutate(
    log_zero_part = log(1 - plogis(betazi)),
    # problemen met log(1/beta), negatieve waarden
    # is dit de beste manier om dit "op te lossen"??
    log_gamma_part = ifelse(1 / beta > 0, log(1 / beta), 0),
    sum_logparts = log_zero_part + log_gamma_part
    ) %>%
  group_by(jaar, SalZone) %>%
  summarize(mzb_tonnage = exp(median(sum_logparts)),
            mzb_tonnage_low = exp(quantile(sum_logparts, 0.025)),
            mzb_tonnage_upp = exp(quantile(sum_logparts, 0.975))
            )
...
hg_salzone_summary %>%
  ...
  mutate(across(starts_with("mzb"),
         ~ .x * totopp / 1e6),
         methode = "hg_replicated",
         totopp_km2 = totopp / 1e6) %>%
  select(-totopp) -> hg_replicated_salzone
```
...
benthos_design %>%
  ...
  summarize(
        ...
        mzb_tonnage = survey_total(
          ...
          vartype = "ci",
          level = 0.95)) %>%
  mutate(methode = "Horvitz-Thomson") -> svytotal_salzone
```

(ref:schattingen-overzichtstabel) Schatting van tonnages biomassa MZB volgens twee methodes in de intertidale zone van de Zeeschelde en Durme opgesplitst per jaar.

```{r schattingen-overzichtstabel}
bind_rows(svytotal_jaar, hg_replicated_jaar) %>%
...
(ref:schattingen-overzichtstabel-salzone) Schatting van tonnages biomassa MZB volgens twee methodes in de intertidale zone van de Zeeschelde en Durme opgesplitst per jaar en saliniteitszone.

```{r schattingen-overzichtstabel-salzone}
bind_rows(svytotal_salzone, hg_replicated_salzone) %>%
...
benthos_afdw_raai %>%
  st_drop_geometry() %>%
  count(jaar) %>%
  gt(caption = "(ref:raai-steekproefgrootte-totaal)")
```
...
benthos_raai_design %>%
  ...
  summarise(
    ...
    mzb_tonnage = survey_total(
      ...
      vartype = "ci",
      level = 0.95)) %>%
  gt(caption = "(ref:schattingen-raai)") %>%
  fmt_number(where(is.double), decimals = 1)
```
  ...
  summarise(
    ...
    n = n(),
        mzb_tonnage = survey_total(
          ...
          vartype = "ci",
          level = 0.95)) %>%
  gt(caption = "(ref:schattingen-raai-subtargets)") %>%
  fmt_number(columns = where(is.double), decimals = 1)
```
