...
bind_rows(mrp_raaispatial_ecotoop,
...
### Allocatie van totale steekproefgrootte over strata

We gebruiken beide jaren gecombineerd om de varianties te schatten en gebruiken hiervoor enkel de spatial survey.
Om een variantie of standaarddeviatie goed in te schatten zijn zeer veel data nodig, daarom is het beter om de beide jaren te combineren - alhoewel dit ook jaar tot jaar variatie bevat.
...
Volgens deze resultaten zou ruim de helft van de locaties in Zeeschelde IV vallen.
Deze formule voor allocatie is echter alleen kostenoptimaal voor de schatting van de totale draagkracht van het intertidale systeem.
...
(ref:allocatie) Allocatie van de steekproefgrootte. Kolommen spatial en raai tonen het resultaat van de allocatieformule uitgedrukt als een relatief percentage. De kolom n_locaties toont het resultaat als we deze allocatie toepassen op een totale steekproefgrootte van 150, maar een minimum steekproefgrootte per stratum van 3 vereisen.
...
benthos_design %>%
  group_by(KRWzone, ecotoop_werkelijk.benthos) %>%
  ...
  select(campagne, KRWzone,
         ecotoop = ecotoop_werkelijk.benthos,
         allocatie) %>%
  pivot_wider(names_from = campagne, values_from = allocatie) %>%
  mutate(n_locaties = pmax(3, round(spatial / 100 * 150))) %>%
  gt(groupname_col = "KRWzone",
     caption = "(ref:allocatie)") %>%
  fmt_number(columns = where(is.double),
             decimals = 1) %>%
  summary_rows(groups = TRUE,
               columns = c(raai, spatial),
               fns = list(totaal = ~sum(.)),
               decimals = 1) %>%
  fmt_number(columns = c(n_locaties),
             decimals = 0) %>%
  summary_rows(groups = TRUE,
               columns = c(n_locaties),
               fns = list(totaal = ~sum(.)),
               decimals = 0) %>%
  grand_summary_rows(columns = c(spatial, raai, n_locaties),
                     fns = list(totaal = ~sum(.)),
                     decimals = 0)
```

### Afweging meer locaties versus nauwkeuriger bemonsteren

Om de afweging te maken tussen meer locaties bemonsteren versus per locatie een hogere bemonsteringsinspanning te doen, hebben we informatie nodig over de kosten geassocieerd met de selectie van de locaties en met de bemonstering ter plaatse (inclusief naverwerking in het labo).
Daarnaast hebben we ook informatie nodig die toelaat om de totale variantie op te splitsen in variantie die het gevolg is van het trekken van een steekproef en variantie die het gevolg is van ter plaatse een bemonstering uit te voeren die echter niet foutloos is.
We hebben echter geen informatie die toelaat om deze opdeling in variantiecomponenten te maken.

Tabel \@ref(tab:kosten-selectie-bemonstering) toont dat voor de raaien de bemonsteringskost `r round((0.8 + 5.2) / (1 + 0.3), 1)` maal hoger is dan de selectiekost.
Voor de spatial survey is de bemonsteringskost `r round((1.2 + 5.2) / (1.7 + 0.3), 1)` maal hoger dan de selectiekost.
In deze berekening zitten echter alleen de kostenposten die kunnen uitgedrukt worden in gewerkte uren.
Zaken zoals materiaalkosten zijn niet inbegrepen en dragen bij aan zowel de bemonsteringskost (labomaterialen, ...) als de selectiekost (verplaatsingskosten).
...
(ref:kosten-selectie-bemonstering) Overzicht van bemonsteringskosten en selectiekosten (uitgedrukt in uren per staal).

```{r kosten-selectie-bemonstering}
kosten %>%
  filter(!str_detect(kostenpost, "subtidaal")) %>%
  mutate(type_veldwerk = case_when(
    taakgroep == "veldwerk_raai" ~ "raai",
    taakgroep == "veldwerk_spatial" ~ "spatial",
    TRUE ~ "beide"
  )) %>%
  pivot_wider(names_from = type_veldwerk,
