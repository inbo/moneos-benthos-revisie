...
De hurdle is nodig waarden gelijk aan 0, omdat de gamma verdeling of lognormale verdeling enkel voor strikt positieve continue waarden gedefinieerd is.
...
Een manier om gecorrigeerde, design-based standaardfouten te schatten is door het hurdle-gamma model vele malen opnieuw te fitten met telkens iets andere gewichten.
...
Voor de raaigegevens is het verleidelijk om ook de identiteit van de afzonderlijke raaien in de analyse te betrekken als een random intercept.
Dit is echter problematisch.
Bij een random intercept is er een veronderstelling van uitwisselbaarheid waaraan niet voldaan is bij de raaien aangezien binnen elke raai een gradiënt aanwezig is.
Deze gradiënt maakt dat een locatie binnen een raai niet uitwisselbaar is met een andere locatie binnen dezelfde raai.
...
\sigma^2_{BF} &= \frac{\sigma^2_B}{nS}\\
\sigma^2_{LF} &= \frac{\sigma^2_L}{nS}
\end{align*}
$$
...
De bemonsteringsfout is omgekeerd evenredig met de bemonsteringsinspanning $S$ en de steekproefgrootte.
De term $\sigma^2_B$ stelt de bemonsteringsfout voor bij een eenheid van de bemonsteringsinspanning ($S = 1$ & $n = 1$).
In het geval van MZB is de bemonsteringsinspanning onder andere bepaald door de bemonsterde oppervlakte per staal.
De in het labo gemaakte meetfout $\sigma^2_LF$, hangt af van de keuzes die gemaakt worden bij de naverwerking van de stalen en de invloed daarvan op de herhaalbaarheid van een meting $\sigma^2_L$.
De formules tonen dat het aantal staalnamelocaties $n$ verhogen een gunstig effect heeft op elk van de foutenbronnen.

Voor een optimalisatie van de bemonstering, moeten we op een vergelijkbare manier ook de kosten geassocieerd met selectie, bemonstering en naverwerking van het staal in rekening brengen.
Wanneer we een vereenvoudigd lineair kostenmodel veronderstellen, dan kunnen we de totale kost $K_T$ berekenen als:

$$K_T = n(K_P + S(K_B + K_L))$$.

Hiervoor hebben we de kost geassocieerd met de selectie van één locatie $K_P$ nodig en met de bemonstering van één locatie $K_B$ en het geassocieerde labowerk $K_L$.
Er kan aangetoond worden dat een optimale bemonsteringsinspanning $S_o$ en optimale steekproefgrootte $n_o$ bekomen wordt met volgende formules:

$$\begin{align*}
S_o &= \frac{\sqrt{K_P}}{\sqrt{K_B + K_L}}\frac{\sigma_B + \sigma_L}{\sigma_P} \\
n_o &= \frac{K_T}{K_P}f_P &\text{met  } f_p = \frac{\sqrt{K_P}\sigma_P}{\sqrt{K_P}\sigma_P + \sqrt{K_B + K_L}(\sigma_B + \sigma_L)}
\end{align*}
$$
