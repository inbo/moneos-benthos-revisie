...
Afhankelijk hoe groot je een locatie neemt $M$ en hoeveel subsamples $m$ je neemt per locatie, zal de steekproeffout wijzigen afhankelijk van de varianties tussen en binnen locaties (zie Figuur \@ref(fig:twostage)).
De schatter van variantie op het totaal kan als volgt berekend worden (geval self-weighting design, i.e. constante sample fracties en bovendien ook constante M en m):

$$\hat{\sigma}^2_{\hat{y}} = \frac{N^2}{n}(1-f_1)\hat{s}^2_{1y} + \frac{N^2}{n}\frac{\bar{M}^2}{\bar{m}}(1-f_2)\bar{\hat{s}^2_{2y}}$$
...
varianties_df %>%
  ...
  mutate(
    varcomp1_mean = 1 / n * var_1y_pop,
    varcomp2_mean = 1 / (n * m) * mean_var_2y_pop, 
    var_mean_sample =  varcomp1_mean + varcomp2_mean) %>%
  ggplot(aes(x = proefvlakgrootte / 1000)) +
  geom_line(aes(y = varcomp1_mean)) + 
  geom_line(aes(y = varcomp2_mean)) + 
  geom_line(aes(y = var_mean_sample)) + 
  geom_rug(sides = "b") +
