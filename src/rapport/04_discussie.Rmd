...
twostage <- function(
  ...
  s = 1L, #bemonsteringsinspanning (aantal pixels)
  ...
  pos1 <- floor(nrow * ncol / s / 2) + 1
  if (s > 1) {
    positions <- c(pos1, pos1 + cumsum(rep(pos1, s - 1)))
  } else {
    positions <- pos1
  } 
  ...
  stopifnot(all.equal(sum(samplevec), s))

  local_sample_mean <- raster::focal(
    x = spdistr, 
    w = matrix(data = samplevec,
               nrow = nrow, ncol = ncol) / s,
    fun = sum)
  ...
  local_var <- local_var * (1 - s / (nrow * ncol))
...
varianties_df <- expand_grid(
  ...
  s = c(1, 2, 4, 8)) %>%
  filter(s <= odd_nrows ^ 2) %>%
  group_by(odd_nrows, s) %>%
  summarise(
    twostage(spatial_distribution = landschap, s = s, nrow = odd_nrows)) %>%
