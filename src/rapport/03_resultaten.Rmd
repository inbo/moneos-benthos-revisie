...
De auteurs geven aan dat deze strategie, waarbij transecten gericht volgens een omgevingsgradiënt worden gekozen, aanvaardbaar kan zijn op voorwaarde dat de totale steekproef aantoonbaar representatief is voor de gehele doelpopulatie.
...
Een nadeel: "Gradsects cannot be used to estimate species occurrences per unit area -- for which a stratified random sample design may be needed".
...
benthos_afdw_raai %>%
  # er zijn enkele locaties waar Z (hoogte) veel te hoge waarden heeft
  filter(Z < 20) %>%
  ggplot() + 
