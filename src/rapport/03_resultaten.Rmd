...
# not working
hurdle_gamma2 <- function(y, shape, rate, hu) {
    ifelse(y == 0,
           dbinom(1, 1, hu, log = TRUE),
           dbinom(0, 1, hu, log = TRUE) + dgamma(y, shape, rate, log = TRUE)
           )
}

make_derivative <- function(y, shape, rate, hu) {
  h <- 1e-5
  y_plus_h <- hurdle_gamma2(y + h, shape, rate, hu)
  y_min_h <- hurdle_gamma2(y - h, shape, rate, hu)
  derivative <- (y_plus_h - y_min_h) / (2 * h)
  return(derivative)
}

m <- svymle(
  loglike = hurdle_gamma2,
  gradient = make_derivative,
  design = benthos_design,
  method = "bobyqa",
  lower = list(c(0, 0), c(0), c(0)),
  upper = Inf,
  formulas = list(~tot_afdw,
                  shape = ~jaar,
                  rate = ~1,
                  hu = ~1),
  start = list(c(0.5, 0), c(0.5), c(0.5)))
