...
```{r plot-ruwe-data, fig.cap = "(ref:plot-ruwe-data)"}
benthos_afdw_zeeschelde_durme_rupel %>%
  filter(tidaal != "subtidaal") %>%
  rename(ecotoop_werkelijk = ecotoop_werkelijk.benthos) %>%
  st_drop_geometry() %>%
...
mrp_raaispatial %>%
  ...
  mutate(methode = "MRP") -> mrp_raaispatial_salzone_ecotoop
```

```{r mrp-raaispatial-2-jaar}
# model zonder survey => poststrata ook survey verwijderen anders dubbeltelling
benthos_afdw_poststrat_zonder_survey <- benthos_afdw_poststrat %>%
...
mrp_raaispatial_2 %>%
  add_epred_draws(newdata = benthos_afdw_poststrat_zonder_survey) %>%
  # omzetting naar ton per stratum per jaar
  mutate(.epred = .epred / opp_per_staal_m2 * stratum_opp_m2 / 1e6) %>%
  # sommering naar ton voor gewenste groepering in elk jaar
  group_by(jaar, .draw) %>%
  ...
  rename(mzb_tonnage = .epred,
         mzb_tonnage_low = .lower,
         mzb_tonnage_upp = .upper) %>%
  inner_join(benthos_afdw_poststrat_zonder_survey %>%
               group_by(jaar) %>%
               summarize(n = sum(n),
                         n_raai = sum(n_raai),
...
mrp_raaispatial_2 %>%
  ...
  mutate(methode = "MRP") -> mrp_raaispatial2_salzone_ecotoop
```
...
(ref:schattingen-raaispatial-ecotoop) Schatting van tonnage MZB voor elk jaar in elke saliniteitszone x ecotooptype combinatie met de MRP methode. Twee modellen werden gefit, één waarbij aparte schattingen per type survey bekomen werd en één waarbij het type survey uit het model gelaten werd (= schatting op basis van alle locaties samen ongeacht survey).

```{r schattingen-raaispatial-ecotoop}
bind_rows(mrp_raaispatial_salzone_ecotoop,
          mrp_raaispatial2_salzone_ecotoop) %>%
  select(jaar, SalZone,
         ecotoop_werkelijk,
         survey, methode, n, totopp_km2, starts_with("mzb")) %>%
  group_by(jaar, SalZone) %>%
  mutate(n = as.integer(n)) %>%
...
```{r n-raai-spatial}
benthos_afdw_poststrat_zonder_survey %>%
