...
Afhankelijk hoe je locatie definieert, zal dit $\sigma_P$ (de intrinsieke ruimtelijke variabiliteit) verkleinen ofwel $\sigma_B$ (de variabiliteit bij één eenheid van bemonsteringsinspanning op een bepaalde locatie).
Het zijn een soort van communicerende vaten (zie Figuur \@ref(fig:maup)).

(ref:maup) Illustratie van effect van definitie van locatie op variantiecomponenten. De figuur links is een simulatie van een gebied waarbinnen een vleksgewijze verspreiding van MZB is (in feite voor te stellen als een zich in alle richtingen oneindig uitgestrekt gebied met gelijkaardig ruimtelijk patroon: vergelijk rode zone met ruimer gebied errond). De simulatie gaat uit van 30% bedekking van MZB (en dus biomassa als we veronderstellen dat de biomassa homogeen verdeeld is waar het voorkomt). De figuur rechts geeft de hiermee overeenkomende variantiecomponenten in functie van hoe groot we een locatie definiëren: populatievariantie (popvar), lokale variantie (locvar) en de som van beide (totvar). Bij de berekening is uitgegaan van een vierkant proefvlak als locatie.

```{r maup, out.width="100%", cache=TRUE, fig.cap="(ref:maup)"}
bedekking <- 0.3
...
maup <- function(X, nrow = 3, ncol = nrow) {
  r1 <- raster::raster(as.matrix(X))
  local_mean <- raster::focal(
    x = r1, 
    w = matrix(data = 1 / nrow^2, nrow = nrow, ncol = ncol),
    fun = sum)
  ...
  result <- data.frame(nrow = nrow,
                       popvar = var(local_mean@data@values, na.rm = TRUE),
                       locvar = mean(local_var@data@values, na.rm = TRUE))
  return(result)
}

varianties_df <- data.frame(
  odd_nrows = c(1, 3, 5, 7, 9, 11, 33, 51, 71, 99, 121, 149, 199)) %>%
  group_by(odd_nrows) %>%
  summarise(maup(X = landschap, nrow = odd_nrows)) %>%
  mutate(totvar = popvar + locvar) %>%
  pivot_longer(cols = c(popvar, locvar, totvar),
...
p2 <- varianties_df %>%
  ...
  geom_text(
    data = data.frame(label = "limietgeval = 0.3 * (1 - 0.3) = 0.21",
                      proefvlakgrootte = 0,
                      waarde = 0.3 * (1 - 0.3)),
    aes(label = label),
    nudge_x = 15) +
